package com.apl.form {
	...
	/**
	模拟表单提交 可以提交字段 和图片一起 如果有其他表单参数 getPostData中的parameters设置成指定的Vars就好 uploadDataFieldName为模拟的file的变量名
	...
	public class UploadPostHelper {
		...
		public static function getPostData(fileName:String, byteArray:ByteArray, uploadDataFieldName:String = "filedata", parameters:Object = null):ByteArray {
			...
			bytes = 'Content-Type: application/octet-stream'+';Cookie: uin=andypan;';
			for ( i = 0; i < bytes.length; i++ ) {
				postData.writeByte( bytes.charCodeAt(i) );
			}
			postData = LINEBREAK(postData);
